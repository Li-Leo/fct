...
def make_release(version):
    ...
    my_env["PATH"] = "C:\Program Files\IAR Systems\Embedded Workbench 7.0\common\\bin;" \
                     + my_env["PATH"]
    cmd = "iarbuild" + " IAR_project\\" + "Spump18_slaver.ewp" + " -make Release"
    ...
    if errors[0] == '0':
        ...
        if p.exists():
            ...
            p.mkdir()
        in_file = Path.cwd() / ('IAR_project/Release/exe/' + 'Spump18_slaver.txt')
        shutil.copy(in_file, p)
        rename(p / 'Spump18_slaver.txt', p / (s+'.txt'))
    else:
        print('Build fail!')
