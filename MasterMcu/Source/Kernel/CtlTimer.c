...
typedef struct  
{
    ...
    /*1: normal*/
    /*2: screen's timer, when exit screen, this id will auto stop*/    
    /*4: low power mode timer, this timer can work at low power mode*/   
    //the property can combine with the property
    T_U8 property;
}TS_TIMER;
...
void CtlTimerBindHandle( TE_TIMER_ID id,  TF_TIMER_FUNC func )
{
    ...
    g_timer_mgr[id].property = NORMAL_TIMER;
}
...
void CtlTimerBindScreenHandle( TE_TIMER_ID id,  TF_TIMER_FUNC func )
{
    assert(id<E_TIMER_MAX);
    g_timer_mgr[id].handle_func = func;
    g_timer_mgr[id].property = SCREEN_TIMER;
}
...
void CtlTimerBindLowPowerHandle(TE_TIMER_ID id, TF_TIMER_FUNC func)
{
    assert(id<E_TIMER_MAX);
    g_timer_mgr[id].handle_func = func;
    g_timer_mgr[id].property = LOW_POWER_MODE_TIMER;    
}
...
void CtlTimerClearAllOfScreen(void)
{
    ...
    {
        if(g_timer_mgr[i].property&SCREEN_TIMER)//screen timer
        {
            //clear timer
...
void CtlTimerClearAll(T_U8 timer_poperty)
{
    ...
    for (i=0; i<E_TIMER_MAX; i++)
    {
        if(g_timer_mgr[i].property & timer_poperty)
        {
            g_timer_mgr[i].is_activated = E_FALSE;
            g_timer_mgr[i].is_repeat = E_FALSE;
        }
    }
}
...
void CtlTimerHandle(T_U8 timer_poperty)
{
    ...
    for (i=0; i<E_TIMER_MAX; i++)
    {
        ...
        if(g_timer_mgr[i].is_activated &&
            ...
            (timer_poperty&g_timer_mgr[i].property)
            )
        {
