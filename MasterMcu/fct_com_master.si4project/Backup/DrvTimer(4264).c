...
/******************************************************************************/  
  TA2CTL |= TASSEL__SMCLK|ID_3;//定时器TA2的时钟源为SMCLK，使能定时器中断，停止定
                               //时器工作
  TA2CCTL0 |= CCIE;            //使能定时器TA2CCR0.CCIFG中断
/******************************************************************************/  
  TA3CTL |= TASSEL_2;          //定时器TA3的时钟源为SMCLK，使能定时器中断，停止定
                               //时器工作
...
void DrvTimerCheckAndSetAutoWakeupTimer()
{
    ...
    else
    {
        ...
        g_timer_next_check_rtc_count = CHECK_RTC_INTERVAL_TIME_MS;
